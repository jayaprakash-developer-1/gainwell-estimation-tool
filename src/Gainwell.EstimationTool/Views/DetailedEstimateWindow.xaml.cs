...
public partial class DetailedEstimateWindow : Window
{
    ...
    {
        // Setup BA Test Cases Grid G�� 21 rows matching Excel Dtl BA_Considerations (rows R15-R37)
        ...
        // Setup WTCG��IterationG��Derived propagation for each experience-level group
        SetupWtcGroup(0);   // New to Area (rows 0G��6)
        SetupWtcGroup(7);   // Proficient  (rows 7G��13)
        SetupWtcGroup(14);  // Expert      (rows 14G��20)

        // Setup Regression Testing G�� Proficient hardcoded (matching Excel Row 40)
        ...
        // Setup Production Validation Grid G�� 9 rows matching Excel labels and order
        ...
        // Complexity columns are now separate count inputs G�� no ComboBox ItemsSource needed
    ...
    public void UpdateSummaryTab()
    {
        ...
        // Note: sysDocProdVal and remainingBdd flow directly to Grand Total (col 11 only in Excel) G�� not Adjusted Hrs (col 10)
...
public class BaTestCaseRow : INotifyPropertyChanged
{
    ...
    /// <summary>Complexity Total G�� weighted hours from counts (same as before).</summary>
    ...
    // Display properties G�� blank for Iteration (IsInfoRow) rows, formatted value otherwise
    ...
    private decimal CalculateTotal()
    {
        ...
        if (UseEffectiveCounts)
        {
            ...
            // CT = +�( WTC_count_i +� rate_i +� iteration_i ) G�� WTC+�Iteration already pre-computed as Eff* counts
